...
## Configuration options via `jeasx.config.js` 

The configuration object from `jeasx.config.js` is imported directly into both the build process and server runtime and provides essential options for `esbuild` and `Fastify`, allowing you to use the full power of JavaScript (e.g package imports) to build your advanved configurations.
