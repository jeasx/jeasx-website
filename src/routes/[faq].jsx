...
export default function ({}) {
  return (
    ...
    >
      ...
      <p>
        Yes, Jeasx provides a context object via <code>this</code> which gets
        automagically propagated to sub-components. You can populate the context
        in an endpoint (e.g., a guard):
      </p>
      <Code
        source={`
...
`}
      />
      <p>
        Later, you can access the context in any sub-component which gets called
        via an endpoint:
      </p>
      <Code
        source={`
...
`}
      />
      <p>
        Have a look at the <a href="https://expo.jeasx.dev/context">demo</a> to
        see how things work using the context as them provider.
      </p>
      <hr />
      <h2 id="error-handler">
  ...
  return <h1>Internal server error</h1>;
      ...
      <p>
        For example, if you want to prettify the HTML output, you can wire up a
        response handler in any endpoint (e.g., a guard). The response handler
        takes the resulting payload as a parameter and returns the modified
        payload.
      </p>
      <Code
        source={`
...
`}
      ...
      <p>
        ...
        <a href="https://github.com/jeasx/jeasx-website/blob/main/package.json">
          package.json
        </a>{" "}
      ...
      <p>
        Yes, of course. Jeasx is a server side rendering framework at its heart,
        but it can be easily used to export a static site. In order to do so,
        simply run your application with <code>npm start</code> and then execute{" "}
        <Code
          source={`wget --mirror --page-requisites --no-host-directories --directory-prefix=www http://localhost:3000 http://localhost:3000/404`}
          lang="bash"
        />
        to export your site to a <code>www</code>-directory, which can be easily
        served via a simple file-server.
      </p>
      <p>
        Have a look at the{" "}
        <a href="https://github.com/jeasx/jeasx-website/blob/main/Dockerfile">
          Dockerfile
        </a>{" "}
        of this website to see how things can be wired up for serving a static
        export with <a href="https://caddyserver.com">Caddy</a> as file-server.
      </p>
      <hr />
      ...
      <p>
        ...
        <a href="https://browsersync.io">Browsersync</a> in proxy-mode in a
        second terminal:
      </p>
      <Code
      ...
      <p>
        As Vercel configures the public folder as default directory for static
        assets, you have to specify another directory in your{" "}
        <code>vercel.json</code>. For example, create an empty directory named{" "}
        <code>output</code> at root level with only a <code>.gitkeep</code> in
        it and register it in your <code>vercel.json</code>:
      </p>
      <Code source={`{... "outputDirectory": "output" ...}`} lang="json" />
      <p>
        This change is only recommended, if your Vercel application applies
        appropriate cache control headers (<code>CDN-Cache-Control</code>) or is
