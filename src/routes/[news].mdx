...
<Highlight title="Release policy" icon={{ html: IconAttention }}>
Jeasx is dedicated to delivering a highly stable platform for web development. It is recommend reviewing the changelog prior to upgrading your project to be in sync with the latest changes and improvements.

The policy outlined below will help you understand what to expect to change from each released version (formatted as `Major.Minor.Patch`):

**Patch:** No changes to your project are needed.

**Minor:** Slight adjustments to your project's configuration may be needed, usually through the `.env.js` file, for it to function correctly.
