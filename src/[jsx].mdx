...
With Jeasx all HTML markup is escaped by default. If you want to include HTML (or other code) snippets, you can provide an object with the key `html` containing the literal code to be included in the rendered result: `{{ html: "<p>Some HTML from a CMS</p>" }}`.
...
If you need to disable HTML escaping globally or for a component and children, you can set `this.jsxEscapeHTML = false` in a JSX component. This feature can be used for advanced patterns (e.g. to create custom HTML components):
...
The following example adds a timestamp to all image sources, so each image receives a new URL with each deployment. It works by adding a custom `jsxToString` handler via the `this` context. Just add the following code as `src/[guard].js` to your project:
...
export default function ({ request, reply }) {
  this.jsxToString = (jsxElement) => {
    if (jsxElement.type === "tag" && jsxElement.tag === "img") {
      jsxElement.props.src = jsxElement.props.src + "?" + REVISION;
    }
...
<Highlight title="Learn more" icon={{ html: IconAttention }}>
  For all the advanced features provided by the `jsx-async-runtime`, have a look at the [example
  application](https://github.com/jeasx/jsx-async-runtime/blob/main/example/src/App.jsx) in the
  GitHub-Repository.
</Highlight>
...
A good example for a Jeasx website powered by MDX is this website. Have a look at [GitHub](https://github.com/jeasx/jeasx-website) to see how things are wired up.

If you want to use MDX with Jeasx, simply run `npm install @mdx-js/esbuild` and add the configuration below to `ESBUILD_SERVER_OPTIONS` in `jeasx.config.js`.
...
<b>Please note:</b> To access props in MDX, you have to use `props` as explicit namespace (e.g.
`props.request.path`).
