...
<Highlight title="Environment variables for client code" icon={{ html: IconAttention }}>
  If you want to use environment variables in client-side JavaScript, you have to configure them at build time via [ESBUILD_BROWSER_OPTIONS#define](https://esbuild.github.io/api/#define):

```js
export default {
  /** @type {() => import("esbuild").BuildOptions} */
  ESBUILD_BROWSER_OPTIONS: () => ({
    define: { "process.env.API_ENDPOINT": `"https://expo.jeasx.dev/jokes/api/programming"` },
  }),
};
```
